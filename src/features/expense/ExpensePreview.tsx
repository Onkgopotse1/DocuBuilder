        ...
        <div id="expense-section" className="bg-[#ffffff] rounded-3xl overflow-hidden shadow-[0_32px_80px_-12px_rgba(0,0,0,0.6)]" style={{ backgroundColor: '#ffffff', color: '#0f172a' }}>
          ...
          <div className="flex">
            ...
            <div className="flex-1 px-8 py-7 flex justify-between items-start">
              <div>
                ...
                <h1 className="text-2xl font-black text-[#0f172a] tracking-tight">John Mokoena</h1>
                <div className="text-[#64748b] text-sm mt-0.5">Sales · john@email.com</div>
                <div className="text-[#94a3b8] text-xs mt-2">Submitted to: <span className="font-semibold text-[#475569]">Acme Corp</span></div>
              </div>
              <div className="text-right">
                <div className="text-[11px] font-black uppercase tracking-widest text-[#94a3b8] mb-1">Claim No.</div>
                <div className="text-xl font-black text-[#0d9488]">EXP-2025-001</div>
                <div className="text-xs text-[#94a3b8] mt-1">1 Jun – 30 Jun 2025</div>
            ...
            <div className="space-y-1">
              ...
              {expenses.map((row, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 py-3 border-b border-[#f8fafc] items-center">
                  <div className="col-span-1 text-xs text-[#94a3b8] font-medium">{row.date}</div>
                  <div className="col-span-3 flex items-center gap-1.5">
                    <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${catDot[row.category] ?? 'bg-[#cbd5e1]'}`}></div>
                    <span className="text-xs font-semibold text-[#475569]">{row.category}</span>
                  </div>
                  <div className="col-span-6 text-sm text-[#0f172a]">{row.desc}</div>
                  <div className="col-span-2 text-sm font-bold text-[#0f172a] text-right">R{row.amount.toFixed(2)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mx-8 mb-6 rounded-2xl border-2 border-[#a5f3fc] bg-[#ecfeff] px-6 py-4 flex justify-between items-center">
            <div>
              ...
              <div className="text-xs text-[#94a3b8] mt-0.5">{expenses.length} items · June 2025</div>
            </div>
            <span className="text-3xl font-black text-[#0d9488]">R{total.toFixed(2)}</span>
          </div>
          ...
          <div className="mx-8 mb-8 grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="border-t-2 border-[#e2e8f0] pt-2">
                <div className="text-xs font-semibold text-[#64748b]">John Mokoena</div>
              </div>
            </div>
            <div>
