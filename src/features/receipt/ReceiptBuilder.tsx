...
export default function ReceiptBuilder() {
  ...
  const addItem = () => {
    const newItem: ReceiptItem = { description: "", quantity: 1, unitPrice: 0 };
    setDocument((prev) => ({
      ...prev,
      receipt: { ...prev.receipt, items: [...prev.receipt.items, newItem] },
    }));
  };

  const updateItem = (index: number, field: keyof ReceiptItem, value: any) => {
    setDocument((prev) => {
      const items = [...prev.receipt.items];
      items[index] = { ...items[index], [field]: value };
      return { ...prev, receipt: { ...prev.receipt, items } };
    });
  };
  ...
  const removeItem = (index: number) => {
    setDocument((prev) => ({
      ...prev,
      receipt: { ...prev.receipt, items: prev.receipt.items.filter((_, i) => i !== index) },
    }));
  };
