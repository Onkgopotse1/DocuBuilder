    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 font-['Inter',system-ui,-apple-system,sans-serif] py-8 px-4 sm:px-6 text-slate-800">
      <div className="max-w-4xl mx-auto pt-20">
        ...
        <div className="bg-white rounded-3xl shadow-2xl shadow-slate-300/50 p-6 md:p-10 border border-slate-100">
          <h2 className="text-lg font-semibold text-slate-500 flex items-center gap-2 border-b border-slate-100 pb-4 mb-8 uppercase tracking-wider">🔍 Live Invoice Preview</h2>
          ...
          <div className="invoice-document bg-white">
            <div className="preview-header flex justify-between items-start mb-10 flex-wrap gap-6">
              ...
              <div className="invoice-meta text-right space-y-1">
                ...
                <div className="text-lg font-bold text-slate-800">{invoice.invoiceNumber || "N/A"}</div>
                <div className="text-sm text-slate-500 mt-3">Date Issued</div>
                <div className="font-semibold text-slate-800">{invoice.invoiceDate || "N/A"}</div>
              </div>
            </div>
            ...
            <div className="client-section mb-10 bg-slate-50/80 p-6 rounded-2xl border border-slate-100">
              ...
              <div className="text-lg font-bold text-slate-800">{invoice.clientName || "N/A"}</div>
              <div className="text-slate-600 mt-1">{invoice.clientEmail || "N/A"}</div>
              <div className="text-slate-600 mt-1 leading-relaxed">{invoice.clientAddress || "N/A"}</div>
            </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {invoice.items.map((item, i) => (
                    <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-4 px-6 text-right text-slate-600">R {item.unitPrice.toFixed(2)}</td>
                      <td className="py-4 px-6 text-right font-semibold text-slate-800">R {(item.quantity * item.unitPrice).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
